...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("market_tick", (data) => {
      setTrafficData((prev) => {
        if (prev.signal !== "ACTIVE" || !prev.direction || !prev.entryPrice)
          return prev;
        const pts =
          prev.direction === "CE"
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-[#0a0a0c] border border-gray-800 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[9px] uppercase text-gray-600 border-b border-gray-800">
                ...
                condorData.map((row, i) => (
                  <React.Fragment key={i}>
                    <tr className="border-b border-gray-800/30">
                      ...
                      <td className="text-center font-mono text-xs text-orange-400">
                        ₹{row.call.firefight}
                      </td>
                      ...
                      <td
                        className={`text-right pr-5 font-black text-base ${parseFloat(row.totalPnL) >= 0 ? "text-emerald-400" : "text-red-400"}`}
                        rowSpan="2"
                      >
                        ₹{row.totalPnL}
                      </td>
            ...
            {trafficData.signal === "ACTIVE" ? (
              <div className="grid grid-cols-2 gap-3 flex-1">
                ...
                <Stat
                  ...
                  sub="spot estimate · updates on tick"
                />
                <Stat
                  label={
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-3 py-4">
                <div
                  className={`text-5xl font-black ${trafficData.signal === "CLOSED" ? "text-gray-600" : "text-yellow-500/40"}`}
                >
                  {trafficData.signal}
                </div>
                {trafficData.breakoutHigh > 0 && (
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-800/60 shrink-0">
              ...
              <div className="ml-auto flex items-center gap-1">
                ...
                {logs.length > 0 && (
                  <button
                    onClick={() => setLogs([])}
                    className="px-2 py-0.5 rounded text-[9px] text-gray-700 hover:text-red-400 ml-1 transition-all"
                  >
                    Clear
                  </button>
                )}
          ...
          <div className="bg-[#0a0a0c] border border-gray-800 rounded-2xl overflow-hidden shadow-xl">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-800/60">
              ...
              <div className="ml-auto flex items-center gap-2">
                <span className="text-[8px] text-gray-700 uppercase tracking-wide">
                  PnL source:
                </span>
