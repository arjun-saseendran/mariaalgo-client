...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // ✅ FIX 2: /api/auto-condor/status returns {entryDone, gapOpenDone, gapOpenHold}, not {active}.
        // Derive autoMode from entryDone flag.
        if (aRes.ok) {
  ...
  const toggleAutoMode = async () => {
    ...
    try {
      if (autoMode) {
        // Reset the day state — clears entryDone/gapOpenDone/gapOpenHold
        const res = await fetch(`${IC_URL}/api/auto-condor/reset`, {
          method: "POST",
        });
        if (res.ok) setAutoMode(false);
      } else {
        // Trigger immediate entry check
        const res = await fetch(`${IC_URL}/api/auto-condor/trigger`, {
          method: "POST",
        });
        if (res.ok) setAutoMode(true);
      }
    } catch {
    } finally {
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-3.5 border-b border-slate-800/80 bg-[#08080c]/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span
              className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#08080c] ${connected ? "bg-emerald-500" : "bg-red-500"}`}
            />
          </div>
          <div>
                  ...
                  <div key={i} className="space-y-3">
                    ...
                    <div className="flex items-center justify-between bg-[#0d0d10] rounded-xl px-4 py-3 border border-slate-800/50">
                      ...
                      <div className="text-right">
                        ...
                        <div
                          className={`text-2xl font-black font-mono ${upstoxStatus === "error" ? "text-slate-700" : pnlPos ? "text-emerald-400" : "text-red-400"}`}
                        >
                          {/* ✅ FIX 4: API field is `pnl` not `totalPnL` */}
                      ...
                      <div className="bg-[#0d0d10] border border-red-900/20 rounded-xl p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          ...
                          <div>
                            ...
                            <div
                              className={`font-mono text-xs font-bold ${upstoxStatus === "error" ? "text-slate-700" : ""}`}
                            >
                              {upstoxStatus === "error"
                                ? "—"
                                : `₹${row.call.current}`}
